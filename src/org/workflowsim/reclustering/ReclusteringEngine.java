...
public class ReclusteringEngine {
    ...
    private static List DRReclustering(List<Job> jobList, Job job, int id, List<Task> allTaskList) {
        ...
        double theta = FailureParameters.getGenerator(job.getHostId(), job.getDepth()).getMLEMean();
        
        double phi_gamma = FailureParameters.getGenerator(job.getHostId(), job.getDepth()).getLikelihoodPrior();
