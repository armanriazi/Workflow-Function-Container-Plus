...
public class FailureGenerator {
     ...
     */
    private static final int maxFailureSizeExtension = 1000;
    ...
    public static boolean generate(Job job) {
        ...
        try {
            ...
            for (Task task : job.getTaskList()) {
                int failedTaskSum = 0;
                if (checkFailureStatus(task, job.getHostId())) {
                    //this task fail
                    jobFailed = true;
                    failedTaskSum++;
                    task.setCloudletStatus(Cloudlet.FAILED);
                }
                FailureRecord record = new FailureRecord(0, failedTaskSum, task.getDepth(), 1, job.getHostId(), task.getCloudletId(), job.getUserId());
