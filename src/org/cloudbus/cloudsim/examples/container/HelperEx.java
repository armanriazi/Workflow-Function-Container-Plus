...
/*
    ...
    public static List<ContainerCloudlet> createContainerCloudletList0(int brokerId, String inputFolderName, int numberOfCloudlets)
...
//            int containerType = new RandomGen().getNum(WFCConstants.CONTAINER_TYPES);
...
//            int containerType = 0;
            ...
            containers.add(new PowerContainer(IDs.pollId(Container.class), brokerId, (double) WFCConstants.CONTAINER_MIPS[containerType], WFCConstants.
                    ...
                    new ContainerCloudletSchedulerDynamicWorkload(WFCConstants.CONTAINER_MIPS[containerType],
                            WFCConstants.CONTAINER_PES[containerType]), WFCConstants.WFC_DC_SCHEDULING_INTERVAL));
        }
    ...
    // create the containers for hosting the cloudlets and binding them together.
    public static List<ContainerVm> createVmList0(int brokerId, int containerVmsNumber) {
        ArrayList containerVms = new ArrayList();

        for (int i = 0; i < containerVmsNumber; ++i) {
            ArrayList peList = new ArrayList();
//            int vmType = new RandomGen().getNum(WFCConstants.VM_TYPES);
//            Log.print(vmType);
//            Log.print("\n");
...
//            int vmType = 1;
            ...
            for (int j = 0; j < WFCConstants.VM_PES[vmType]; ++j) {
                peList.add(new ContainerPe(j, new CotainerPeProvisionerSimple((double) WFCConstants.VM_MIPS[vmType])));
            }
            containerVms.add(new PowerContainerVm(IDs.pollId(ContainerVm.class), brokerId, (double) WFCConstants.VM_MIPS[vmType], (float) WFCConstants.VM_RAM[vmType],
                    WFCConstants.VM_BW, WFCConstants.VM_SIZE, "Xen", new ContainerSchedulerTimeSharedOverSubscription(peList),
                    new ContainerRamProvisionerSimple(WFCConstants.VM_RAM[vmType]),
                    new ContainerBwProvisionerSimple(WFCConstants.VM_BW), peList, WFCConstants.WFC_DC_SCHEDULING_INTERVAL));
    ...
    public static List<ContainerHost> createHostList0(int hostsNumber) {
        ArrayList hostList = new ArrayList();
        for (int i = 0; i < hostsNumber; ++i) {
//            int hostType =  new RandomGen().getNum(WFCConstants.HOST_TYPES);
...
//            int hostType = 2;
            ...
            for (int j = 0; j < WFCConstants.HOST_PES[hostType]; ++j) {
                peList.add(new ContainerVmPe(j, new ContainerVmPeProvisionerSimple((double) WFCConstants.HOST_MIPS[hostType])));
            }

//            hostList.add(new PowerContainerHost(i, new ContainerVmRamProvisionerSimple(WFCConstants.HOST_RAM[hostType]),
//                    new ContainerVmBwProvisionerSimple(1000000L), 1000000L, peList, new ContainerVmSchedulerTimeSharedOverSubscription(peList), WFCConstants.HOST_POWER[hostType]));
            hostList.add(new PowerContainerHostUtilizationHistory(IDs.pollId(ContainerHost.class), new ContainerVmRamProvisionerSimple(WFCConstants.HOST_RAM[hostType]),
                    new ContainerVmBwProvisionerSimple(1000000L), 1000000L, peList, new ContainerVmSchedulerTimeSharedOverSubscription(peList), WFCConstants.HOST_POWER[hostType]));
        }
    ...
    public static WFCScheduler createBroker(double overBookingFactor) {
        WFCScheduler broker = null;

        try {
            broker = new WFCScheduler("Broker", overBookingFactor);
...
//    }
    // Data Center
//    public static WFCDatacenter createDatacenter(String name, Class<? extends WFCDatacenter> datacenterClass, List<ContainerHost> hostList, ContainerVmAllocationPolicy vmAllocationPolicy, ContainerAllocationPolicy containerAllocationPolicy, String experimentName, String logAddress) throws Exception {
//        String arch = "x86";
//        String os = "Linux";
//        String vmm = "Xen";
//        double time_zone = 10.0D;
...
//                    WFCConstants.SCHEDULING_INTERVAL, experimentName, logAddress);
//        } catch (Exception e) {
//            e.printStackTrace();
//            System.exit(0);
//        }
