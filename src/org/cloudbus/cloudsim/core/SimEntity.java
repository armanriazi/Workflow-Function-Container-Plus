...
import static org.cloudbus.cloudsim.core.CloudSim.getNetworkDelay;
...
public abstract class SimEntity implements Cloneable {
	...
	public void run() {
		...
		while (ev != null) { 
                        ...
                        if(ev.getData() !=null && ev.getData().equals(null)==false)                       
                                    dataLength=ev.getData().toString().length();

                       if(WFCConstants.CAN_PRINT_SEQ_LOG){
                       Log.printLine("SimEntity=>ev != null=>proccessEvent() By=> "+ " Source:"+ev.getSource()+ " Dest:" + ev.getDestination()+ "  Tag:"+ev.getTag()+" Data:");                            
