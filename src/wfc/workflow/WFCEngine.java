...
import static wfc.examples.WFCExample.hostList;
import static wfc.examples.WFCExample.createHostList;
import static wfc.examples.WFCExample.createContainerList;
...
public final class WFCEngine extends SimEntity {
    ...
    public WFCEngine(String name) throws Exception {
        this(name, WFCConstants.WFC_NUMBER_SCHEDULER);
    }
     ...
     */
    /*
    public void submitVmList(List<? extends ContainerHost> list, int schedulerId) {
        getScheduler(schedulerId).submitVmList(list);
    }

    public void submitVmList(List<? extends ContainerHost> list) {
        //bug here, not sure whether we should have different workflow schedulers
        getScheduler(0).submitVmList(list);
        setVmList(list);
    }
    */
    public List<? extends ContainerHost> getAllHostList(){
        if(hostList != null && ! hostList.isEmpty()){
            ...
            for(int i = 0;i < getSchedulers().size();i ++){
                list.addAll(getScheduler(i).getContainerHostList());
            }
            return list;
        }
    }
     ...
     *
     * @param <T> the generic type
     * @param hostList the new vm list
     */
    private <T extends ContainerHost> void setHosList(List<T> hostList) {
        hostList = hostList;
    }
     ...
     *
     * @param <T> the generic type
     * @param hostList the new scheduler list
     */
     ...
     *
     * @param <T> the generic type
     * @param hostList the new scheduler id list
     */
